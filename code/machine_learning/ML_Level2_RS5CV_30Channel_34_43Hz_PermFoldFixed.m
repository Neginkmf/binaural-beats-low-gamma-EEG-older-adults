%% LEVEL 2 - REPEATED STRATIFIED 5-FOLD CV
% 30 channel-wise 34-43 Hz features. Standalone script.
...
filename=resolve_file('06_NarrowGamma_34_43Hz_Changes_dB_vs_PRE.xlsx'); Tall=readtable(filename,'VariableNamingRule','preserve');
gammaVars=strcat(CHANNELS,'_Gamma34_43'); assert(isequal(Tall.Properties.VariableNames(6:35),gammaVars),'30-channel order check failed.');
fprintf('\n%s - RS5CV ML LEVEL 2 - PERMUTATION-FOLD FIXED\n',METHOD_TAG);
for cc=1:numel(CONTRASTS)
    ...
    T=apply_exclusions(T,exclusions); T=sortrows(T,'SubjectUID'); Y=double(T.Label); X=table2array(T(:,gammaVars)); check_binary_data(X,Y);
    ...
    R=table(string(METHOD_TAG),string(contrast),"Level2_30ch34_43",N,sum(Y==0),sum(Y==1),30,K,NREPEATS,NPERM,meanAUC,sdAUC,100*meanAcc,100*sdAcc,100*meanBal,100*sdBal,100*meanSens,100*meanSpec,pPerm,nullCenter,nullSD, ...
    ...
    disp(R); out=sprintf('%s_RS5CV_PermFoldFixed_Level2_%s',METHOD_TAG,contrast); writetable(R,[out '_summary.csv']); writetable(Rep,[out '_repeat_metrics.csv']); SubjectOrder=table(string(T.SubjectUID),Y,'VariableNames',{'SubjectUID','Label'}); writetable(SubjectOrder,[out '_subject_order.csv']); save([out '_details.mat'],'foldID','Rep','nullMeanAUC','meanAUC','pPerm','SubjectOrder');
end
